...
<nav class="navbar navbar-expand-lg navbar-custom">
    ...
    <script>
        ...
        function confirmarEliminacion(id_genero) {
            // Realizar una petición AJAX para verificar si existen películas asociadas al género
            $.ajax({
                type: "POST",
                url: "../Modelo/verificar_pelis_asociadas.php",
                data: { id_genero: id_genero },
                success: function(response) {
                    if (response === "true") {
                        // Mostrar SweetAlert indicando que no se puede eliminar el género
                        Swal.fire({
                            title: "No se puede eliminar",
                            text: "Existen películas asociadas a este género.",
                            icon: "error",
                            confirmButtonColor: "#3085d6",
                            confirmButtonText: "Aceptar"
                        });
                    } else {
                        // Si no hay películas asociadas, confirmar la eliminación
                        Swal.fire({
                            title: "¿Estás seguro?",
